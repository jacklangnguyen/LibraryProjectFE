...
export class BookComponent implements OnInit {
  ...
  objects: IBOOK[] | null | undefined;
  selectedItem!: IBOOK | any;
  keyWord = '';
  ...
  doubleClickRow(any?: any, any2?: any) {
    // @ts-ignore
    this.router.navigate(['/book/edit/' + any.id]);
  }

  // tslint:disable-next-line:typedef
  selectedRowFun(item: IBOOK) {
  ...
  delete() {
    if (this.selectedItem && this.selectedItem.id) {
      // call api xoa;
      this.bookService.delete([this.selectedItem.id]).subscribe(res => {
        if (res.code === 200) {
